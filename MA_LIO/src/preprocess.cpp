...
void Preprocess::oust64_handler(const sensor_msgs::PointCloud2::ConstPtr &msg, int lidar)
{
  ...
  pl_surf.header.stamp = time_stamp;
  // cout << "===================================" << endl;
...
void Preprocess::hesai_handler(const sensor_msgs::PointCloud2::ConstPtr &msg, int lidar)
{
  pl_surf.clear();
  pl_corn.clear();
  pl_full.clear();
  ...
  double time_stamp = msg->header.stamp.toSec();
  pl_surf.header.stamp = time_stamp;
  pl_surf.header.frame_id = msg->header.frame_id;
  // cout << "===================================" << endl;
  // printf("Pt size = %d, N_SCANS[lidar] = %d\r\n", plsize, N_SCANS[lidar]);
  maximum_time = -9999;
  for (int i = 0; i < pl_orig.points.size(); i++)
  {
    ...
    added_pt.normal_y = 0;
    added_pt.normal_z = 0;
    added_pt.curvature = (pl_orig.points[i].timestamp - time_stamp) * time_unit_scale; // curvature unit: ms
    if (maximum_time < added_pt.curvature)
    {
      maximum_time = added_pt.curvature;
    }
